...
def PrepareDatas(FilePath, Headers, IsTrain=True):
    ...
    CombinedText = LoadedDatas['Title']
    if IsTrain:
    ...
    for Epoch in tqdm(range(Epochs), desc="Training Epochs"):
      ...
      #  wandb.log({"Epoch": Epoch, "Train Loss": AvgTrainLoss, "Validate Loss": AvgValidateLoss, "Validate AUC": AUC})
        torch.save(TrainingModel.state_dict(),
                   f'TrainedModel/HeadOnlyERNIE/LearnRate{LearnRate}_Epoch{Epoch + 1}.pth')
        scheduler.step(AvgValidateLoss)
   # wandb.finish()


def EvaluateModel(LearnRate, Epoch, PredictThreshold=0.5):
    ModelFile = f'TrainedModel/HeadOnlyERNIE/LearnRate{LearnRate}_Epoch{Epoch}.pth'
    ...
    ResultPath = f'PredictedLabel/HeadOnlyERNIE/Predictions{PredictThreshold}_LearnRate{LearnRate}_Epoch{Epoch}.csv'
