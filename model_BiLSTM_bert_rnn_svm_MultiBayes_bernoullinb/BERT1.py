...
#模型训练
def set_seed(seed: int):#提供一致的随机数生成，帮助确保实验结果的一致性和可重复性。
    random.seed(seed)
...
set_seed(123)#随意设置整数，不同的种子值会生成不同的随机数序列
...
new_df['Report Content'] = new_df['Report Content'].apply(cleaning) # 修改这里
new_df['Ofiicial Account Name'] = new_df['Ofiicial Account Name'] # 修改这里
